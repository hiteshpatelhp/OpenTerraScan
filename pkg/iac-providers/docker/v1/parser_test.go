	...
	tests := []struct {
	...
	}{
		{
			...
			want: []DockerConfig{
				...
				{Cmd: "ENV", Value: "DEFAULT_OPENTERRASCAN_VERSION 1.5.1", Line: 5},
				{Cmd: "ENV", Value: "PLANFILE tfplan", Line: 6},
				{Cmd: "ADD", Value: "setup.sh openterrascan.sh launch-atlantis.sh entrypoint.sh /usr/local/bin/", Line: 7},
				{Cmd: "RUN", Value: "mkdir -p /etc/atlantis/ &&     chmod +x /usr/local/bin/*.sh &&     /usr/local/bin/setup.sh", Line: 8},
				{Cmd: "Copy", Value: "openterrascan-workflow.yaml /etc/atlantis/workflow.yaml", Line: 11},
				{Cmd: "USER", Value: "atlantis", Line: 13},
				{Cmd: "ARG", Value: "name=defaultValue", Line: 14},
				{Cmd: "RUN", Value: "openterrascan init", Line: 15},
