	...
	Context("scan iac directories violating k8s policies", func() {
		...
		Context("iac type is specified as k8s", func() {
			It("should scan and display violations in human output format", func() {
				...
				scanUtils.RunScanAndAssertGoldenOutputRegex(openterrascanBinaryPath, filepath.Join(k8sGoldenRelPath, "kubernetes_ingress_human.txt"), helper.ExitCodeThree, false, true, outWriter, errWriter, scanArgs...)
			})
			...
			When("-v flag is used for verbose output", func() {
				It("should display verbose output for human output format", func() {
					...
					scanUtils.RunScanAndAssertGoldenOutputRegex(openterrascanBinaryPath, filepath.Join(k8sGoldenRelPath, "kubernetes_ingress_human_verbose.txt"), helper.ExitCodeThree, false, true, outWriter, errWriter, scanArgs...)
				})
			})
			...
			When("when output type is sarif", func() {
				It("should display violations in sarif format", func() {
					...
					scanUtils.RunScanAndAssertGoldenSarifOutputRegex(openterrascanBinaryPath, filepath.Join(k8sGoldenRelPath, "kubernetes_ingress_sarif.txt"), helper.ExitCodeThree, outWriter, errWriter, scanArgs...)
				})
			})
			...
			When("when output type is json", func() {
				It("should display violations in json format", func() {
					...
					scanUtils.RunScanAndAssertGoldenOutputRegex(openterrascanBinaryPath, filepath.Join(k8sGoldenRelPath, "kubernetes_ingress_json.txt"), helper.ExitCodeThree, false, true, outWriter, errWriter, scanArgs...)
				})
			})
			...
			When("when output type is yaml", func() {
				It("should display violations in yaml format", func() {
					...
					scanUtils.RunScanAndAssertGoldenOutputRegex(openterrascanBinaryPath, filepath.Join(k8sGoldenRelPath, "kubernetes_ingress_yaml.txt"), helper.ExitCodeThree, false, true, outWriter, errWriter, scanArgs...)
				})
			})
			...
			When("when output type is xml", func() {
				It("should display violations in xml format", func() {
					...
					scanUtils.RunScanAndAssertGoldenOutputRegex(openterrascanBinaryPath, filepath.Join(k8sGoldenRelPath, "kubernetes_ingress_xml.txt"), helper.ExitCodeThree, false, true, outWriter, errWriter, scanArgs...)
				})
			})
			...
			When("when output type is junit-xml", func() {
				It("should display violations in junit-xml format", func() {
					...
					scanUtils.RunScanAndAssertGoldenOutputRegex(openterrascanBinaryPath, filepath.Join(k8sGoldenRelPath, "kubernetes_ingress_junit_xml.txt"), helper.ExitCodeThree, true, true, outWriter, errWriter, scanArgs...)
				})
			})
		})
	})
