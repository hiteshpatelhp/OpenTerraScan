...
var _ = Describe("Scan command with rule filtering options", func() {
		...
		Context("--scan-rules options is used", func() {
			Context("single rule is specified via --scan-rules option", func() {
				It("should scan only the rules specified in --scan-rules option", func() {
					...
					scanUtils.RunScanAndAssertJSONOutput(openterrascanBinaryPath, filepath.Join(ruleFilterGoldenRelPath, "scan_single_rule.txt"), helper.ExitCodeThree, false, true, outWriter, errWriter, scanArgs...)
				})
			})
			Context("multiple rules are specified via --scan-rules option", func() {
				It("should scan only the rules specified in --scan-rules option", func() {
					...
					scanArgs := []string{"-p", policyDir, "-i", "terraform", "-d", iacDir, "-o", "json", "--scan-rules", scanRules}
					scanUtils.RunScanAndAssertJSONOutput(openterrascanBinaryPath, filepath.Join(ruleFilterGoldenRelPath, "scan_multiple_rules.txt"), helper.ExitCodeThree, false, true, outWriter, errWriter, scanArgs...)
				})
			})
		})
		...
		Context("--skip-rules options is used", func() {
			Context("single rule is specified via --skip-rules option", func() {
				It("should skip the rule specified in --skip-rules option", func() {
					...
					scanUtils.RunScanAndAssertJSONOutput(openterrascanBinaryPath, filepath.Join(ruleFilterGoldenRelPath, "skip_single_rule.txt"), helper.ExitCodeThree, false, true, outWriter, errWriter, scanArgs...)
				})
			})
			Context("multiple rules are specified via --skip-rules option", func() {
				It("should skip the rules specified in --skip-rules option", func() {
					...
					scanArgs := []string{"-p", policyDir, "-i", "terraform", "-d", iacDir, "-o", "json", "--skip-rules", skipRules}
					scanUtils.RunScanAndAssertJSONOutput(openterrascanBinaryPath, filepath.Join(ruleFilterGoldenRelPath, "skip_multiple_rules.txt"), helper.ExitCodeThree, false, true, outWriter, errWriter, scanArgs...)
				})
			})
		})
		...
		Context("both --scan-rules and --skip-rules are specified", func() {
			Context("single rule is specified via --skip-rules option", func() {
				It("should scan and skip the rules as specified with --scan-rules and --skip-rules option", func() {
					...
					scanUtils.RunScanAndAssertJSONOutput(openterrascanBinaryPath, filepath.Join(ruleFilterGoldenRelPath, "scan_and_skip_rules.txt"), helper.ExitCodeThree, false, true, outWriter, errWriter, scanArgs...)
				})
			})
		})
	...
	Describe("rule filtering via config file", func() {
		...
		Context("config file is specified using OPENTERRASCAN_CONFIG env variable", func() {
			Context("both scan and skip rules are specified", func() {
				JustBeforeEach(func() {
					os.Setenv(openterrascanConfigEnvName, filepath.Join("config", "scan_and_skip_rules.toml"))
				})
				JustAfterEach(func() {
					os.Setenv(openterrascanConfigEnvName, "")
				})
				It("should scan and skip the rules as specified with --scan-rules and --skip-rules option", func() {
			...
			Context("invalid severity is specified in config file", func() {
				It("should error out and exit with status code 1", func() {
					...
					scanUtils.RunScanAndAssertErrorMessage(openterrascanBinaryPath, helper.ExitCodeOne, scanUtils.ScanTimeout, severityLevelIncorrectError, outWriter, errWriter, scanArgs...)
				})
			})
			...
			Context("invalid category is specified in config file", func() {
				It("should error out and exit with status code 3", func() {
					...
					scanArgs := []string{scanUtils.ScanCommand, "-p", policyDir, "-d", iacDir, "-c", configFileAbsPath}
					scanUtils.RunScanAndAssertErrorMessage(openterrascanBinaryPath, helper.ExitCodeOne, scanUtils.ScanTimeout, categoryIncorrectError, outWriter, errWriter, scanArgs...)
				})
			})
		})
	})
	...
	Describe("resource specific rule skipping", func() {
		...
		Context("resource skipping in tf files", func() {
			...
			It("should display violations, skipped violations and exit with status code 3", func() {
				scanArgs := []string{"-p", policyDir, "-i", "terraform", "-d", iacDir, "-o", "json"}
				scanUtils.RunScanAndAssertJSONOutput(openterrascanBinaryPath, filepath.Join(resourceSkipGoldenRelPath, "terraform_file_resource_skipping.txt"), helper.ExitCodeThree, false, true, outWriter, errWriter, scanArgs...)
			})
		})
		...
		Context("resource skipping in k8s files", func() {
			...
			It("should display skipped violations and exit with status code 0", func() {
				...
				scanUtils.RunScanAndAssertJSONOutput(openterrascanBinaryPath, filepath.Join(resourceSkipGoldenRelPath, "kubernetes_file_resource_skipping.txt"), helper.ExitCodeZero, false, true, outWriter, errWriter, scanArgs...)
			})
		})
		...
		Context("resource skipping in docker files", func() {
			...
			It("should display skipped violations and exit with status code 0", func() {
				...
				scanUtils.RunScanAndAssertJSONOutput(openterrascanBinaryPath, filepath.Join(resourceSkipGoldenRelPath, "dockerfile_resource_skipping.txt"), helper.ExitCodeZero, false, true, outWriter, errWriter, scanArgs...)
			})
		})
	})
	Describe("resource specific rule prioritising", func() {
		...
		Context("resource max severity set to Low in tf files", func() {
			...
			It("should display violations with change priority to Low for the resource and exit with status code 3", func() {
				scanArgs := []string{"-p", policyDir, "-i", "terraform", "-d", iacDir, "-o", "json"}
				scanUtils.RunScanAndAssertJSONOutput(openterrascanBinaryPath, filepath.Join(resourcePrioritisingGoldenRelPath, "max_severity_set", "terraform", "terraform_file_setting_max_severity.txt"), helper.ExitCodeThree, false, true, outWriter, errWriter, scanArgs...)
			})
		})
		Context("resource min severity set to High in tf files", func() {
			...
			It("should display violations with change priority to High for the resource and exit with status code 3", func() {
				scanArgs := []string{"-p", policyDir, "-i", "terraform", "-d", iacDir, "-o", "json"}
				scanUtils.RunScanAndAssertJSONOutput(openterrascanBinaryPath, filepath.Join(resourcePrioritisingGoldenRelPath, "min_severity_set", "terraform", "terraform_file_setting_min_severity.txt"), helper.ExitCodeThree, false, true, outWriter, errWriter, scanArgs...)
			})
		})
		Context("resource max severity set to none in tf files", func() {
			...
			It("should skip all violations for the resource and exit with status code 0 since only one resource is in tf file", func() {
				scanArgs := []string{"-p", policyDir, "-i", "terraform", "-d", iacDir, "-o", "json"}
				scanUtils.RunScanAndAssertJSONOutput(openterrascanBinaryPath, filepath.Join(resourcePrioritisingGoldenRelPath, "max_severity_set_none", "terraform", "terraform_file_setting_max_severity_none.txt"), helper.ExitCodeZero, false, true, outWriter, errWriter, scanArgs...)
			})
		})
		...
		Context("resource min severity set to High and skip rule provided in tf files", func() {
			...
			It("should display skipped violations and violations with change priority to High as specified by min severity for the resource and exit with status code 3", func() {
				scanArgs := []string{"-p", policyDir, "-i", "terraform", "-d", iacDir, "-o", "json"}
				scanUtils.RunScanAndAssertJSONOutput(openterrascanBinaryPath, filepath.Join(resourcePrioritisingGoldenRelPath, "min_severity_with_skip_rule", "terraform", "terraform_file_setting_min_severity_with_skip_rule.txt"), helper.ExitCodeThree, false, true, outWriter, errWriter, scanArgs...)
			})
		})
		...
		Context("resource max severity set to Low in k8s files", func() {
			...
			It("should display violations with change priority to Low for the resource and exit with status code 3", func() {
				...
				scanUtils.RunScanAndAssertJSONOutput(openterrascanBinaryPath, filepath.Join(resourcePrioritisingGoldenRelPath, "max_severity_set", "k8s", "k8s_file_setting_max_severity.txt"), helper.ExitCodeThree, false, true, outWriter, errWriter, scanArgs...)
			})
		})
		Context("resource min severity set to High in k8s files", func() {
			...
			It("should display violations with change priority to High for the resource and exit with status code 3", func() {
				scanArgs := []string{"-p", policyDir, "-i", "k8s", "-d", iacDir, "-o", "json"}
				scanUtils.RunScanAndAssertJSONOutput(openterrascanBinaryPath, filepath.Join(resourcePrioritisingGoldenRelPath, "min_severity_set", "k8s", "k8s_file_setting_min_severity.txt"), helper.ExitCodeThree, false, true, outWriter, errWriter, scanArgs...)
			})
		})
		Context("resource max severity set to none in k8s files", func() {
			...
			It("should skip all violations for the resource and exit with status code 0", func() {
				scanArgs := []string{"-p", policyDir, "-i", "k8s", "-d", iacDir, "-o", "json"}
				scanUtils.RunScanAndAssertJSONOutput(openterrascanBinaryPath, filepath.Join(resourcePrioritisingGoldenRelPath, "max_severity_set_none", "k8s", "k8s_file_setting_max_severity_none.txt"), helper.ExitCodeZero, false, true, outWriter, errWriter, scanArgs...)
			})
		})
		...
		Context("resource min severity set to High and skip rule provided in k8s files", func() {
			...
			It("should display skipped violations with change priority to High as specified by min severity for the resource and exit with status code 0", func() {
				scanArgs := []string{"-p", policyDir, "-i", "k8s", "-d", iacDir, "-o", "json"}
				scanUtils.RunScanAndAssertJSONOutput(openterrascanBinaryPath, filepath.Join(resourcePrioritisingGoldenRelPath, "min_severity_with_skip_rule", "k8s", "k8s_file_setting_min_severity_with_skip_rule.txt"), helper.ExitCodeZero, false, true, outWriter, errWriter, scanArgs...)
			})
		})
		Context("resource max severity set to Low in dockerfile", func() {
			...
			It("should give violations for the resource and exit with status code 3", func() {
				...
				scanUtils.RunScanAndAssertJSONOutput(openterrascanBinaryPath, filepath.Join(resourcePrioritisingGoldenRelPath, "max_severity_set", "docker", "dockerfile_max_severity_low.txt"), helper.ExitCodeThree, false, true, outWriter, errWriter, scanArgs...)
			})
		})
		Context("resource min severity set to High in dockerfile", func() {
			...
			It("should give violations for the resource and exit with status code 3", func() {
				scanArgs := []string{"-p", policyDir, "-i", "docker", "-d", iacDir, "-o", "json"}
				scanUtils.RunScanAndAssertJSONOutput(openterrascanBinaryPath, filepath.Join(resourcePrioritisingGoldenRelPath, "min_severity_set", "docker", "dockerfile_min_severity_high.txt"), helper.ExitCodeThree, false, true, outWriter, errWriter, scanArgs...)
			})
		})
		Context("resource max severity set to None in dockerfile", func() {
			...
			It("should skip violations for the resource and exit with status code 0", func() {
				scanArgs := []string{"-p", policyDir, "-i", "docker", "-d", iacDir, "-o", "json"}
				scanUtils.RunScanAndAssertJSONOutput(openterrascanBinaryPath, filepath.Join(resourcePrioritisingGoldenRelPath, "max_severity_set_none", "docker", "dockerfile_max_severity_none.txt"), helper.ExitCodeZero, false, true, outWriter, errWriter, scanArgs...)
			})
		})
