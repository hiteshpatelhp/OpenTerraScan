...
import (
	...
	helpUtils "github.com/tenable/openterrascan/test/e2e/help"
...
var _ = Describe("Help", func() {
	...
	Describe("openterrascan is run without any command", func() {
		It("should print all supported commands and exit with status code 0", func() {
			session = helper.RunCommand(openterrascanBinaryPath, outWriter, errWriter)
			helpUtils.ValidateExitCodeAndOutput(session, helper.ExitCodeZero, filepath.Join("golden", "help_command.txt"), true)
		})
	})

	Describe("openterrascan is run -h flag", func() {
		It("should print all supported commands and exit with status code 0", func() {
			session = helper.RunCommand(openterrascanBinaryPath, outWriter, errWriter, "-h")
			helpUtils.ValidateExitCodeAndOutput(session, helper.ExitCodeZero, filepath.Join("golden", "help_flag.txt"), true)
		})
	})

	Describe("openterrascan is run with an unknown command", func() {
		It("should exit with status code 1 and display a error message", func() {
			session = helper.RunCommand(openterrascanBinaryPath, outWriter, errWriter, "test")
	...
	Describe("help is run", func() {
		Context("with no arguments", func() {
			It("should print the openterrascan help and exit with status code 0", func() {
				session = helper.RunCommand(openterrascanBinaryPath, outWriter, errWriter, helpCommand)
				helpUtils.ValidateExitCodeAndOutput(session, helper.ExitCodeZero, filepath.Join("golden", "help_command.txt"), true)
			})
		})
		...
		Context("for init command", func() {
			It("should print help for init and exit with status code 0", func() {
				session = helper.RunCommand(openterrascanBinaryPath, outWriter, errWriter, helpCommand, "init")
		...
		Context("for scan command", func() {
			It("should print help for scan and exit with status code 0", func() {
				session = helper.RunCommand(openterrascanBinaryPath, outWriter, errWriter, helpCommand, "scan")
		...
		Context("for server command", func() {
			It("should print help for server and exit with status code 0", func() {
				session = helper.RunCommand(openterrascanBinaryPath, outWriter, errWriter, helpCommand, "server")
		...
		Context("for version command", func() {
			It("should print help for version and exit with status code 0", func() {
				session = helper.RunCommand(openterrascanBinaryPath, outWriter, errWriter, helpCommand, "version")
		...
		Context("for an unknown command", func() {
			It("should display that help topic is not available for entered command and exit with status code 0", func() {
				session = helper.RunCommand(openterrascanBinaryPath, outWriter, errWriter, helpCommand, "test")
